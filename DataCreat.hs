...
convertFields map' = foldlWithKey' upd [] (mapKeys' $ toHashMap map')
 where
    upd list' key' val' 
        | isObject val' =
                (   
                     (convertObject key' val')
                , 
		     (      (mkName $  key'),
			    NotStrict,
			    (   mkValType val' key'    )
        ...
        | otherwise =
                (   
                     (Control.Monad.State.get >>= Control.Monad.State.put)
                , 
		     (      (mkName $  key'),
			    NotStrict,
			    (   mkValType val' key'    )
...
convertObject name' json' = do
    Control.Monad.State.modify $ (Prelude.++) [DataD
                ...
                (mkName name')
                []
                [ RecC (mkName name')  (Prelude.map (snd) (convertFields $ json')) ]
                [mkName "Show", mkName "Eq"] ]
